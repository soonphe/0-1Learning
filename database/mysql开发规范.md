...
- 使用tinyint代替enum、bit等类型，而后是smallint（2位），值域比较大的情况用int（4）
- 使用decimal代替double、float等容易丢失精度的数据类型
- 避免使用字符串存储日期，尽量使用timestamp和datetime等日期类型（datetime直观，timestamp存储时间范围小，性能更好）
